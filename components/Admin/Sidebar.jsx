...
function Sidebar() {
  ...
  const sidebarData = [
    ...
    },
    {
      title: "lists",
      items: [
        { name: "users", icon: <PersonOutlineIcon /> },
    ...
    },
    {
      title: "useful",
      items: [
        { name: "status", icon: <InsertChartIcon /> },
              ...
              {item.items.map((link, i) => (
                <Link
                  href={
                    link.name === "dashboard"
                      ? `/dashboard`
                      : link.name === "status" || link.name === "notifications"
                      ? `/dashboard`
                      : `/dashboard/${link?.name}`
                  }
                  key={i}
                  className={
                    router.asPath.endsWith(link?.name?.toLowerCase())
                      ? `dashboard__sidebar__parts__singlePart__items__item active`
                      : `dashboard__sidebar__parts__singlePart__items__item`
                  }
                >
                  <div
                    className={`dashboard__sidebar__parts__singlePart__items__item__icon`}
                  >
                    {link.icon}
                  </div>
                  <div
                    className={`dashboard__sidebar__parts__singlePart__items__item__name`}
                  >
                    {link.name}
                  </div>
                </Link>
